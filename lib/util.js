...
function errorHandler(err, req, res, next) {
    ...
    (req.logger || req.app.logger).log(`${level}/${component}`, errForLog(errObj));
...
function createRouter(opts) {
 ...
 /**
  * Adds logger to the request and logs it. Middleware.
  * @param {Object} req - Express request object
  * @param {Object} res - Express response object
  * @param {Function} next - Express next middleware
  */
function initAndLogRequest(req, res, next) {
    req.headers = req.headers || {};
    req.headers['x-request-id'] = req.headers['x-request-id'] || generateRequestId();
    req.logger = req.app.logger.child({
        request_id: req.headers['x-request-id'],
        request: reqForLog(req, req.app.conf.log_header_whitelist)
    });
