...
function setErrorHandler(app) {
    ...
    app.use((err, req, res, next) => {
        ...
        if (!errObj.status) { errObj.status = 500; } // eslint-disable-line max-statements-per-line
        if (!errObj.type) { errObj.type = 'internal_error'; } // eslint-disable-line max-len,max-statements-per-line
        // add the offending URI and method as well
        if (!errObj.method) { errObj.method = req.method; } // eslint-disable-line max-len,max-statements-per-line
        if (!errObj.uri) { errObj.uri = req.url; } // eslint-disable-line max-statements-per-line
